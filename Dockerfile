...
# Parser Directives
#
# A parser directive must be specified before any instruction or comment and
# can only be specified once. It is ignored if anything else comes before it.
#
# escape directive:
...
# escape a newline. This allows a Dockerfile instruction to span multiple lines.
# Note that regardless of whether the escape parser directive is included in a
# Dockerfile, escaping is not performed in a RUN command, except at the end of a line.
# Setting the escape character to ` is especially useful on Windows, where \ is the
# directory path separator. ` is consistent with Windows PowerShell.
